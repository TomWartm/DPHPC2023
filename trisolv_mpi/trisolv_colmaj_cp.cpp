...
int main(int argc, char** argv) {
	...
	/*
	MPI_Datatype row_seg;
	MPI_Type_vector(NDEF, std_rows, NDEF - std_rows, MPI_DOUBLE, &row_seg);
	MPI_Type_commit(&row_seg);*/
	if (rank == 0) {
		...
		for (int n = 1; n < size; ++n) {
			for (int j = 0; j < NDEF; ++j) {
				/*std::cout << "{";
				for (int k = 0; k < std_rows; ++k) std::cout << A[n * std_rows + j * NDEF + k] << " ";
				std::cout << "}\n";*/
