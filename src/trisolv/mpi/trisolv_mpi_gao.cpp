...
double trisolv_mpi_gao(int size, int rank, int N, double*& A, double*& x, double*& b, void (*init)(int, double*, double*, double*)) {
	if (N <= 256)
		return trisolv_naive(size, rank, N, A, x, b, init);
	else if (N <= 16384 * 2)
		return trisolv_mpi_gao_block(size, rank, N, A, x, b, init, 8);
	else
		return trisolv_mpi_gao_block(size, rank, N, A, x, b, init, 1);
}
...
double trisolv_mpi_gao_block(int size, int rank, int N, double*& A, double*& x, double*& b, void (*init)(int, double*, double*, double*), int block_size) {
    int rows, A_size;
