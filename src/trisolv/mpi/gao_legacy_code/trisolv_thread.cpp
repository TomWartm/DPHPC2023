...
void subtract(int rows, int rank_x_std_rows, int j_x_A_size, double* A, double x, double* b) {
    for (int i = 0; i < rows; ++i) {
		b[rank_x_std_rows + i] -= A[j_x_A_size + i] * x;
	}
}
...
//    	std::cout << "THREADS: " << std::thread::hardware_concurrency() << "\n";
    ...
    for (int j = 0; j < NDEF; j += 2) {
		...
		for (int k = 0; k < 2; ++k) {
			threads.push_back(std::thread(subtract, rows, rank_x_std_rows, (j + k) * A_size, A, x[j + k], b));
		}
		for (int k = 0; k < 2; ++k) {
			threads[k].join();
		}
		
//		subtract(rows, rank_x_std_rows, j_x_A_size, A, x[j], b);
        /*for (int i = 0; i < rows; ++i) {
